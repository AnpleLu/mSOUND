function [p_time] = Forward3D(mgrid, medium, excit_ps, sensor_mask,...
                    reflection_order, varargin)
% DESCRIPTION:
% Simpson-like intergration in the forward direction 

% USAGE:
% [p_time] = Forward3D(mgrid, medium, excit_ps, sensor_mask,...
%                      reflection_order, varargin)
...
MDM_ani = 0;  % flag for animation
MDM_mov = 0;  % flag for recording animation
if ~isempty(varargin)
    for input_index = 1:length(varargin)
        switch varargin{input_index}
            ...
            p_ani(:,:,1) = excit_p(:,:,round(mgrid.num_y/2));
            figure
            imagesc (mgrid.z*1e3, mgrid.x*1e3, squeeze(p_ani(1,:,:)))
            xlabel ('x (mm)')
            ylabel ('y (mm)')
...
%% calculate non-reflecting layer properties
if MDMA ==1
...
elseif medium_cond == 2
    ...
    for I = 2:mgrid.num_z+1
        ...
        M  = repmat(M_nonlinear, 1, mgrid.num_x, mgrid.num_y).*...
             fftshift(fftn(f.*f));
        F1 = excit_F.*exp(0.5*1i*K*mgrid.dz) +...
             0.5*mgrid.dz*M.*exp(0.5*1i*K*mgrid.dz)./(2i.*K);  % P01(z+mgrid.dz/2)
        F1(isnan(F1)) = 0;
        F1(real(Ktemp1)<=0) = 0; 
        ...
        M1 = repmat(M_nonlinear, 1, mgrid.num_x, mgrid.num_y).*...
             fftshift(fftn(f.*f));
        clear f
        F2 = excit_F.*exp(0.5*1i*K*mgrid.dz) +...
             0.25*mgrid.dz*exp(0.5*1i*K*mgrid.dz)./...
             (2i.*K).*(M + M1./exp(0.5*1i*K*mgrid.dz));% P12(z+mgrid.dz/2)
        clear M1
        F2(isnan(F2)) = 0;
        F2(real(Ktemp1)<=0) = 0;   
        ...
        M2 = repmat(M_nonlinear, 1, mgrid.num_x, mgrid.num_y).*...
             fftshift(fftn(f.*f));
        clear f
        F3 = F2.*exp(0.5*1i*K*mgrid.dz) +...
             0.5*mgrid.dz*M2.*exp(0.5*1i*K*mgrid.dz)./(2i.*K);  % P03(z+mgrid.dz)
        F3(isnan(F3)) = 0;
        F3(real(Ktemp1)<=0) = 0;  
        ...
        M3 = repmat(M_nonlinear, 1, mgrid.num_x, mgrid.num_y).*...
             fftshift(fftn(f.*f));
        clear f
        F4 = F2.*exp(0.5*1i*K*mgrid.dz) + ...
             0.25*mgrid.dz.*(M2 + M3./exp(0.5*1i*K*mgrid.dz)).*...
             exp(0.5*1i*K*mgrid.dz)./(2i.*K); % P14(z+mgrid.dz)
        clear M3   F2   
        F4(isnan(F4)) = 0;
        F4(real(Ktemp1)<=0) = 0;    
        ...
        clear f
        F5 = excit_F.*exp(1i*K*mgrid.dz) + ...
             mgrid.dz/6.0.*(M + 4*M2./exp(0.5*1i*K*mgrid.dz) +...
             M4./exp(1i*K*mgrid.dz)).*...
...
elseif medium_cond == 4 && MDMC == 0
    ...
    disp ('Linear medium without transmission correction')
    % prepare for the calculation of evanescent wave
    Ktemp1_kx = zeros(1, mgrid.num_x);
    Ktemp1_ky = zeros(1,1,mgrid.num_y);
...
%% reflection
        if reflection_order~=0
            ...
            p_ref(:,:,:, I-1) = real(ifftn(ifftshift(P_in))).*(T_rhoc-1);
            clear T_rhoc P_in 
        end
...
elseif medium_cond ==4 && MDMC == 1 % add correction  
    ...
    disp ('Linear medium with transmission correction')
...
%     disp ('nonlinear media with corrections')        
    % calculate the M term on RHS of solution equation
...
%% add phase correction  
        ...
        clear rho11 rho22 rho1   
        f = f.*T_rhoc;
%         f = f.*repmat(T_rhoc, mgrid.num_t, 1,1);
        F5 = fftshift(fftn(f)); 
...
if reflection_order ~=0 && (reflection_option ==1)
    if MDMC == 1
        ...
        reflection = MReflection3D(mgrid, medium, p_ref, ...
                     M_linear, K, cw, sensor_mask, reflection_order);
        p_time = p_time + reflection;  
    else
        
        reflection = Reflection3D(mgrid, medium, p_ref, ...
                     M_linear, K, cw, sensor_mask, reflection_order);
        p_time = p_time + reflection;  
    end
