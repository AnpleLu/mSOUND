function [M_nonlinear, M_linear, Ktemp, Ktemp2, cw] = Mterm2D_MMDM(mgrid, medium, medium_cond)
% DESCRIPTION:
% calculate the M term at RHS of solution equation

% USAGE:
% [M_nonlinear, M_linear, Ktemp, Ktemp2, cw] = Mterm2D_MMDM(mgrid, medium, medium_cond)
...
if medium_cond == 4  % 'linear media'
    ...
    M_nonlinear = 0;
    cw          = zeros(mgrid.num_t, mgrid.num_x, mgrid.num_y+1);
    M_linear    = zeros(mgrid.num_t, mgrid.num_x, mgrid.num_y+1);
    Ktemp       = zeros(mgrid.num_t, mgrid.num_x);
    ...
    for it = 1:mgrid.num_t
        ...
        Ktemp(it,:)     = squeeze((mgrid.w(it).^2./medium.c0.^2 ) - mgrid.kx.^2);  
        Ktemp2(it,:,:)  = (wt.^2./medium.c.^2 ) - mgrid.kx.'.^2*ones(1,mgrid.num_y+1); 
...
elseif medium_cond == 5 % 'nonlinear media'
    ...
    M_nonlinear = zeros(mgrid.num_t, mgrid.num_x, mgrid.num_y+1);
    cw          = zeros(mgrid.num_t, mgrid.num_x, mgrid.num_y+1);
    M_linear    = zeros(mgrid.num_t, mgrid.num_x, mgrid.num_y+1);
    Ktemp       = zeros(mgrid.num_t, mgrid.num_x);
    Ktemp2      = zeros(mgrid.num_t, mgrid.num_x, mgrid.num_y+1);
    ...
    for it = 1:mgrid.num_t
        ...
        Ktemp(it,:)      = squeeze((mgrid.w(it).^2./medium.c0.^2 ) - mgrid.kx.^2);  
%         Ktemp2(it,:,:)   = (wt.^2./medium.c.^2 ) - mgrid.kx.'.^2*ones(1,mgrid.num_y+1); 
        Ktemp2(it,:,:)   = (wt.^2./cwt.^2 ) - mgrid.kx.'.^2*ones(1,mgrid.num_y+1); 
