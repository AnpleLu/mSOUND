function [p_reflection3] = BMReflection2D(mgrid, medium, p_ref, ...
...
% USAGE:
% [p_reflection3] = BMReflection2D(mgrid, medium, p_ref, ...
%                   M_linear,K, K2, cw, sensor_mask, ...
%                   reflection_order, excit_ps, M_nonlinear,...
%                   MDMS, medium_cond)
...
for iref = 1:reflection_order
    ...
    if mod(iref,2)==1
        ...
        for I = mgrid.num_y:-1:1 % the layer at which the main waveform arrives
            ...
            p_ref2(:,:,I) = real(ifft2(ifftshift(excit_F))).*(T_rhoc-1);
            p_ref3(:,:,I) =  p_ref3(:,:,I) + p_ref2(:,:,I);
...
if medium_cond == 4 && MDMS == 1
    ...
    for I = mgrid.num_y:-1:1
        ...
        T_rhoc2 = 2.*rho_rho(:,I+1).*c_c(:,I+1)./...
            (rho_rho(:,I+1).*c_c(:,I+1) + rho_rho(:,I).*c_c(:,I));  %%layered medium        
        
        if  (max(max(abs(T_rhoc2))) ~=1 || min(min(abs(T_rhoc))) ~=1) 
            f = f - p_ref3(:,:,I);       
        end
        excit_F = fftshift(fft2(f));         
...
elseif medium_cond == 5 && MDMS == 1
    ...
    for I = mgrid.num_y:-1:1
        ...
        if  (max(max(abs(T_rhoc2))) ~=1 || min(min(abs(T_rhoc2))) ~=1) 
            f = f - p_ref3(:,:,I);       
        end
        excit_F = fftshift(fft2(f)); 
        ...
        wx2 = (mgrid.w.*mgrid.w).'*ones(1,mgrid.num_x);
        e1 = exp(1i*(K - K2(:,:,I))*backward_dy);
        k_dif2 = (wx2./medium.c0./medium.c0 - wx2./cw(:,:,I)./cw(:,:,I))*backward_dy;      
        k_dif  = (wx2./medium.c0./medium.c0 - wx2./cw(:,:,I+1)./cw(:,:,I+1))*backward_dy; 
        clear  wx2  
        correction = excit_F.*exp(1i*K2(:,:,I)*backward_dy).*...
                    (1-e1.*(1+k_dif./4i./K)./(1-k_dif2./4i./K));
