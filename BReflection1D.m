function [p_reflection3] = BReflection1D(mgrid, medium, p_ref, ...
...
% USAGE:
% [p_reflection3] = BReflection1D(mgrid, medium, p_ref, ...
%                   M_linear, K, cw, sensor_mask,...
%                   reflection_order, excit_ps, M_nonlinear,...
%                   MDMS, medium_cond)
    ...
    for I = mgrid.num_x:-1:1     
        ...
        if sum(sensor_mask(I))~=0
            ...
            p_reflection(:,sensor_mask_I1:sensor_mask_I2) = ...
                p_reflection(:,sensor_mask_I1:sensor_mask_I2) + p_I;
...
if medium_cond == 4 && MDMS == 1
   for I = mgrid.num_x:-1:1  
        ...
        rho1 = rho_rho(I+1);
        rho2 = rho_rho(I);
        T_rhoc2 = 2.*rho1.*c1./(rho1.*c1 + rho2.*c2);  %%layered medium   
