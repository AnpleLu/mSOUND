function [M_nonlinear, M_linear, cw] = Mterm2D(mgrid, medium, medium_cond)
% DESCRIPTION:
...
elseif medium_cond == 2 % 'nonlinear homgoeneous medium'
    ...
    M_nonlinear = single(zeros(mgrid.num_t, 1));
    M_linear    = single(zeros(mgrid.num_t, 1));
    ...
    for it = 1:mgrid.num_t
        ...
        M_nonlinear(it, :) = medium.beta0./(medium.rho0)./cw(it,:).^4.*wt.^2;
        
        % unit is dB/cm and then transformed to Np/m
        alpha_Np = abs(medium.ca0.*((mgrid.w(it))/2/pi/1e6).^medium.cb0)*100*0.1151; 
        
        M_linear(it,:) = abs(2*alpha_Np.*(medium.c0.^3)./((mgrid.w(it)).^2))./...
                         medium.c0.^4.*1i.*(mgrid.w(it)).^3 ;        
    end
    
    
elseif medium_cond == 3 % 'nonlinear homgoeneous medium'
...
elseif medium_cond == 4 % 'linear medium'
